...
(() => {
 ...
 const form=document.querySelector('#siteSearchForm, #searchForm'),input=document.querySelector('#siteSearchInput, #searchInput'),results=document.querySelector('#searchResults, #results');
 if(form&&input&&results){fetch('/search-index.json').then(r=>r.json()).then(index=>{
  const normalize=x=>String(x||'').toLowerCase();
  const render=q=>{q=q.trim();if(!q){results.innerHTML='<p class="note">Search Education, Products, Travel or useful guides.</p>';return}
   const words=normalize(q).split(/\s+/).filter(Boolean);
   const ranked=index.map(x=>{const hay=normalize([x.title,x.description,(x.keywords||[]).join(' ')].join(' '));const exact=normalize(x.title).includes(normalize(q))?3:0;const score=words.reduce((n,w)=>n+(hay.includes(w)?1:0),0)+exact;return {...x,score}}).filter(x=>x.score).sort((a,b)=>b.score-a.score || String(a.title).localeCompare(String(b.title)));
  ...
  document.querySelectorAll('[data-query]').forEach(btn=>btn.addEventListener('click',()=>{input.value=btn.getAttribute('data-query')||'';render(input.value);input.focus();history.replaceState(null,'','/search/?q='+encodeURIComponent(input.value))}));
